...
public class SpreadSheetImporterSpec {
    ...
    public void xls() throws FileNotFoundException {
        ImportResult<Person> resolve =
            SpreadSheetImporter.create(new FileInputStream(FileUtil.brotherPath(this.getClass(), "/2003.xls")))
                // 表头行数
                .skip(1)
                // sheet索引
                .sheet(0)
    ...
    public void xlsx() throws FileNotFoundException {
        ImportResult<Person> resolve =
            SpreadSheetImporter.create(new FileInputStream(FileUtil.brotherPath(this.getClass(), "/2010.xlsx")))
                .skip(1)
                .sheet(0)
                .resolve(Person.class);
        // 是否存在校验不通过的数据
    ...
    public void repeated() throws FileNotFoundException {
        ImportResult<Person> resolve =
            SpreadSheetImporter.create(new FileInputStream(FileUtil.brotherPath(this.getClass(), "/repeated.xlsx")))
                // 表头行数
                .skip(1)
                // 快速失败
                .failFast(true)
